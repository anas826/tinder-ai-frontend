import { User, MessageCircle, Heart, X, Send } from 'lucide-react';
import './App.css'
...
const fetchConversation = async(conversationId) =>{
  console.log("fetching conversationId: "+conversationId)
...
const ChatScreen = ({currentMatch, conversation, refreshState}) => {
  ...
  const handleSend = async (conversation, input) =>{
    if(input.trim()){
      const response = await sendMessage(conversation.id, input);
      setInput('');
    }
    refreshState();
  }
